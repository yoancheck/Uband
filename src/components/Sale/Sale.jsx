...
export default function Sale({ data, showButtons, remove }) {
  const {
    title,
    instrument,
    price,
  ...
  const isLikedByMe = () => {
    return userLikes.includes(user.uid);
  };
  ...
  const deletePost = () => {
    const db = getFirestore(app);
    deleteDoc(doc(db, "sales", data.id));
    remove();
  };
    ...
    <Card className={classes.root} style={{ marginTop: 10 }}>
      <CardActions className={classes.action}>
        ...
        <CardContent style={{ width: "100%" }}>
          ...
          <div>
            {showComments && (
              <CommentSection comments={comments} data={data} isSale={true} />
            )}
          </div>
        </CardContent>
      </CardActions>
